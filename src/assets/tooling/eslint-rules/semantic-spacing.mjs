export const semanticSpacing = {
  ...
  create(context) {
    ...
    function isVisuallyMultiline(node) {
      ...
      if (node.type === 'VariableDeclaration') {
        return node.declarations.some(
          (declarator) =>
            declarator.init && declarator.init.loc.start.line !== declarator.init.loc.end.line
        );
      }
    ...
    function hasBlankLineBefore(node) {
      ...
      const textBetween = context.sourceCode.text.slice(tokenBefore.range[1], node.range[0]);
    ...
    function checkBlock(node) {
      ...
      statements.forEach((statement, index) => {
        ...
        if (isVisuallyMultiline(predecessor) && !hasBlankLineBefore(statement)) {
          context.report({
            node: statement,
        ...
        if (
          ...
          context.report({
            node: statement,
            message: 'Unexpected blank line between explaining-const and return.',
