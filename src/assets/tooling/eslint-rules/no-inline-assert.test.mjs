...
describe('no-inline-assert', () => {
  it('passes when both arguments are named identifiers', () => {
    ruleTester.run('no-inline-assert', noInlineAssert, {
      valid: [
        { code: 'assert.strictEqual(actual, expected);', filename: TEST_FILENAME },
        { code: 'assert.ok(actualId);', filename: TEST_FILENAME },
        { code: 'assert.rejects(actual, expected);', filename: TEST_FILENAME },
        { code: 'assert.deepStrictEqual(actualItems, expectedItems);', filename: TEST_FILENAME },
      ],
      invalid: [],
    });
  });
  ...
  it('skips non-test files', () => {
    ruleTester.run('no-inline-assert', noInlineAssert, {
      valid: [{ code: 'assert.strictEqual(result.pass, true);', filename: 'src/lib/util.mjs' }],
      invalid: [],
    });
  });
  ...
  it('exempts string literal message as last argument of assert.ok', () => {
    ruleTester.run('no-inline-assert', noInlineAssert, {
      valid: [
        { code: 'assert.ok(isPresent, "must be present");', filename: TEST_FILENAME },
        { code: 'assert.equal(actual, expected, "must match");', filename: TEST_FILENAME },
      ],
      invalid: [],
    });
  });
